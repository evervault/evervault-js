...
test.describe("evervault inputs", () => {
  test.describe("v2 render localized inputs with custom fonts", () => {
    test.beforeEach(async ({ page }) => {
      await page.goto(
        `http://localhost:3020/?team=${teamUuid}&app=${appUuid}&fontUrl=${fontUrl}&fontFamily=${fontFamily}&labelWeight=${labelWeight}&labelFontSize=${labelFontSize}&inputFontSize=${inputFontSize}`
      );
      expect(await page.locator("id=font-url")).toBeTruthy();
    });
    ...
    test("inserts google fonts preconnect tags", async ({ page }) => {
      await expect(await page.locator("[id=font-preconnect]")).toBeTruthy();
      await expect(
        await page.locator("[id=font-preconnect-cors]")
      ).toBeTruthy();
    });

    test("asserts that LabelFontSize is set to 24px", async ({ page }) => {
      await expect(await page.locator("label").filter({ hasText: 'Expiration date'})).toHaveCSS(
        "font-size",
        "24px"
      );
    });
    ...
    test("asserts that inputFontSize is set to 30px", async ({ page }) => {
      await expect(await page.getByLabel("Expiration date")).toHaveCSS(
        "font-size",
        "30px"
      );
    });

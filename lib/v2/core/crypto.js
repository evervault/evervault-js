...
module.exports = (config, isDebug) => {
  ...
  const _encryptFile = async (
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.readAsArrayBuffer(dataLocation);
    });
  };
  ...
  const _formatFile = async (
    ...
    const compressedKey = cryptoUtils.ecPointCompress(exportableEcdhPublicKey);
    const evEncryptedFileIdentifier = Buffer.from([0x25, 0x45, 0x56, 0x45, 0x4e, 0x43]);
    const versionNumber = Buffer.from([0x00]);
    const offsetToData = Buffer.from([0x00, 0x37]);

      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (id || name) {
            if (currentInput.hasAttribute('data-encrypt')) {
                ...
                if (id) input.id = id.replace('ev-hidden-', '');
                if (name) input.name = name.replace('ev-hidden-', '');
                form.appendChild(input);                

                if (currentInput.id && currentInput.id.substr(0,10) !== 'ev-hidden-') currentInput.id = `ev-hidden-${id}`;
                currentInput.name = '';

                form[id || name].value = encryptedValue;
                formPayload[id ? id.replace('ev-hidden-', '') : name.replace('ev-hidden-', '')] = encryptedValue;
            } else {

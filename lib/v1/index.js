...
class EvervaultClient {
  constructor(teamId, appId, customConfig = {}) {
    ...
    this.config = Config(teamId, appId, customConfig.urls);
    ...
    this.http = Http(this.config.http, this.config.teamId, this.config.appId, this.context);
  ...
  inputs(id, settings) {
    try {
      if (typeof settings === "string") {
        ...
        const { theme, ...remainingSettings } = settings || {};
        return this.input.generate(id, theme, remainingSettings);
      }
    } catch (err) {
      throw err;
    }

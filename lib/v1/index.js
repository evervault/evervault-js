...
class EvervaultClient {
  constructor(teamId, customConfig = {}) {
    ...
    // fetch keys on boot
    this.http.getCageKey().then(this.updateKeys);
  }

  async updateKeys(cageKey) {
    ...
    this.defineHiddenProperty(
      "_derivedAesKey",
      await deriveSharedSecret(keyPair, this._ecdhTeamKey, this._ecdhPublicKey)
    );
  }
  ...
  async encrypt(data) {
    ...
    if (!Datatypes.isDefined(this._ecdhTeamKey)) {
      ...
      await this.updateKeys(cageKeyResponse);
    }
    try {
  ...
  inputs(id, settings) {
    try {
      const theme = typeof settings === "object" ? settings.theme : settings;

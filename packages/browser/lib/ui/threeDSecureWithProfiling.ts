...
interface ThreeDSecureWithProfilingEvents {
  ready: () => void;
  success: () => void;
  failure: () => void;
...
export default class ThreeDSecureWithProfiling {
  #session: string;
  #isOverlay: boolean;
  ...
  #events = new EventManager<ThreeDSecureWithProfilingEvents>();
  ...
  constructor(
    ...
    this.#frame = new EvervaultFrame(client, "ThreeDSecureWithProfiling");
    this.#client = client;
  ...
  async #updateOutcome(
    ...
    // For profiled sessions, we still use the same endpoint to update the final outcome
  ...
  on<T extends keyof ThreeDSecureWithProfilingEvents>(
    event: T,
    callback: ThreeDSecureWithProfilingEvents[T]
  ) {

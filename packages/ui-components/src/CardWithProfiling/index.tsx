...
import { BrandIcon } from "../Card/BrandIcon";
import { CardCVC } from "../Card/CardCVC";
import { CardExpiry } from "../Card/CardExpiry";
import { CardHolder } from "../Card/CardHolder";
import { CardNumber } from "../Card/CardNumber";
import { DEFAULT_TRANSLATIONS } from "../Card/translations";
import { useCardReader } from "../Card/useCardReader";
import {
  changePayload,
  collectIcons,
  isAcceptedBrand,
...
import { useEarlyProfiling } from "./useEarlyProfiling";
import type { CardForm, CardConfig } from "../Card/types";
import type {
  CardField,
...
export interface CardWithProfilingConfig extends CardConfig {
...
export function CardWithProfiling({ config }: { config: CardWithProfilingConfig }) {
  ...
  const {
    ...
    enabled: config.enableEarlyProfiling ?? true,
    onComplete: config.onProfilingComplete,
    onError: config.onProfilingError,
  });
    ...
    onChange: (formState) => {
      const triggerChange = async () => {
        ...
        const cardData = await changePayload(ev, formState, fields, {
          ...
          fingerprintId: profilingSession?.sessionId,
        });
  ...
  return (
    <>
      <fieldset
        ...
        ev-fields={fields}
        ev-profiling-active={isProfilingActive ? "true" : "false"}
      >

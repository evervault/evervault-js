...
async function setupVariant(variant: VariantKey) {
  ...
  function maybeAdvance() {
    ...
    const transitions = [
      // network fetch + parse of the iframe document
      { phase: "boot", from: phaseStartMark, to: "load" },
      // the iframe's own JS booting, up to its handshake with the parent
      { phase: "js", from: "load", to: "handshake" },
      // the card app rendering its fields after the handshake
